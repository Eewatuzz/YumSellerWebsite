...
import { setPersistence, browserLocalPersistence } from "https://www.gstatic.com/firebasejs/12.15.0/firebase-auth.js";
import { onAuthStateChanged } from "https://www.gstatic.com/firebasejs/12.15.0/firebase-auth.js";


// Global state for the merchant dashboard
const state = {
    promptPayNumber: '0812345678', // editable default (display-only on this page)
    allOrders: []
};
...
async function loginStaff(email, password) {
  ...
  try {
    ...
    // ผ่านแล้วค่อยเปิดหน้าจัดการ
    showMerchantPanel();

    // เริ่มดึงออเดอร์ (พี่ค่อยเรียกฟังก์ชัน listen ของพี่ตรงนี้)
    listenToOrdersForMerchant(); 
  } catch (e) {
...
// Manual refresh button - the listener below is already real-time,
// this simply re-attaches it (useful if the connection ever drops silently)
window.loadOrdersFromDatabase = function() {
    listenToOrders();
    showToast('🔄 กำลังซิงค์ออเดอร์ล่าสุด...');
};
...
window.savePromptPay = function() {
    ...
    showToast('✅ บันทึกเบอร์พร้อมเพย์ร้านเรียบร้อยแล้ว! (หมายเหตุ: ต้องอัปเดตเบอร์นี้ในไฟล์ order.js ด้วย เพื่อให้ลูกค้าเห็นเบอร์เดียวกัน)');
};
