        ...
        import { getFirestore, collection, doc, setDoc, updateDoc, deleteDoc, query, onSnapshot, getDocs, orderBy, serverTimestamp } from "https://www.gstatic.com/firebasejs/12.15.0/firebase-firestore.js";
        

        // Global state and references variables setup
        const state = {
            currentStep: 1,
            viewMode: 'customer', // 'customer' or 'merchant'
            promptPayNumber: '0812345678', // editable default
            order: {
        ...
        async function initFirebase() {
            try {
                ...
                // ถ้าพี่ไม่ได้ใช้ Custom Token จริง ๆ ให้ตัดส่วนนี้ทิ้งไปได้
                // แล้วใช้ anonymous อย่างเดียวก่อนเพื่อให้เขียน orders ได้
            ...
            // ควรเรียกหลัง init สำเร็จ (และ db/auth มีค่าแล้ว)
            listenToOrders();
        }
        ...
        function listenToOrders() {
            ...
            const handleIncomingData = (docs) => {
                const orders = [];
                docs.forEach(doc => {
                    orders.push(doc.data());
                });

                // Sort orders by timestamp DESC natively on CPU (Strict Rule 2 query optimization)
            ...
            if (db) {
                // Real-time server stream (Strict Rule 2 keeps queries flat)
                ...
                // Fetch local storage mock data on changes
                setInterval(() => {
                    const localData = JSON.parse(localStorage.getItem('yum_mock_orders') || '[]');
                    handleIncomingData({ forEach: (cb) => localData.forEach(d => cb({ data: () => d })) });
                }, 2000);
            }
        }
        ...
        function renderMerchantDashboard() {
            ...
            // Setup Badge notifications
            const badge = document.getElementById('order-badge');
            if (countPending > 0) {
                badge.innerText = countPending;
        ...
        window.updateOrderStatus = async function(orderId, newStatus) {
            try {
                if (db) {
                    ...
                    const offlineCollection = JSON.parse(localStorage.getItem('yum_mock_orders') || '[]');
                    const orderIndex = offlineCollection.findIndex(o => o.orderId === orderId);
                    if (orderIndex > -1) {
                        offlineCollection[orderIndex].status = newStatus;
                        localStorage.setItem('yum_mock_orders', JSON.stringify(offlineCollection));
                    }
                }
        ...
        // Utility textual and styling badges
        ...
        window.zoomSlip = function(base64Src) {
        ...
        window.switchView = function(view) {
            ...
            const mercView = document.getElementById('merchant-view');
            const btnCust = document.getElementById('btn-customer-mode');
            const btnMerc = document.getElementById('btn-merchant-mode');

            if (view === 'customer') {
                custView.classList.remove('hidden');
                mercView.classList.add('hidden');
                btnCust.className = "px-4 py-1.5 rounded-full text-xs md:text-sm font-medium transition-all duration-300 bg-white text-orange-600 shadow-sm";
                btnMerc.className = "px-4 py-1.5 rounded-full text-xs md:text-sm font-medium transition-all duration-300 hover:bg-white/10 text-white";
                showToast('🛒 ยินดีต้อนรับเข้าสู่หน้าปรุงสลัดสั่งยำ!');
            } else {
                custView.classList.add('hidden');
                mercView.classList.remove('hidden');
                btnMerc.className = "px-4 py-1.5 rounded-full text-xs md:text-sm font-medium transition-all duration-300 bg-white text-orange-600 shadow-sm";
                btnCust.className = "px-4 py-1.5 rounded-full text-xs md:text-sm font-medium transition-all duration-300 hover:bg-white/10 text-white";
                showToast('🏪 ยินดีต้อนรับหลังบ้านร้านยำสดแซ่บ');
                renderMerchantDashboard();
            }
        };

        window.togglePromptPayEdit = function() {
            const display = document.getElementById('pp-display-num');
            const editBox = document.getElementById('pp-edit-box');
            display.classList.add('hidden');
            editBox.classList.remove('hidden');
        };

        window.savePromptPay = function() {
            const newVal = document.getElementById('pp-input').value.trim();
            if (newVal.length < 10) {
                showToast('⚠️ หมายเลขพร้อมเพย์ไม่ถูกต้อง', 'error');
                return;
            }
            state.promptPayNumber = newVal;
            document.getElementById('pp-display-num').innerText = newVal;
            document.getElementById('shop-promptpay-display').innerText = newVal;
            
            // Toggle view back
            document.getElementById('pp-display-num').classList.remove('hidden');
            document.getElementById('pp-edit-box').classList.add('hidden');
            showToast('✅ บันทึกเบอร์พร้อมเพย์ร้านเรียบร้อยแล้ว!');
        };
